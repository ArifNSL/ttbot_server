...
// const stunServers = [{
//     urls: [
//         'turn:3.110.28.227:3478?transport=udp',
//         'turn:3.110.28.227:3478?transport=tcp',
//         'turns:3.110.28.227:5349?transport=tcp'
//     ],
//     username: 'username1',
...
const localVideo3 = document.querySelector('#local-video3');
const localVideo4 = document.querySelector('#local-video4');
const localVideo5 = document.querySelector('#local-video5');
...
var localStream3 = new MediaStream();
var localStream4 = new MediaStream();
var localStream5 = new MediaStream();
...
// buttons to toggle self audio and video 3
btnToggleAudio3 = document.querySelector("#btn-toggle-audio3");
btnToggleVideo3 = document.querySelector("#btn-toggle-video3");

// buttons to toggle self audio and video 4
btnToggleAudio4 = document.querySelector("#btn-toggle-audio4");
btnToggleVideo4 = document.querySelector("#btn-toggle-video4");

// buttons to toggle self audio and video 4
btnToggleAudio5 = document.querySelector("#btn-toggle-audio5");
btnToggleVideo5 = document.querySelector("#btn-toggle-video5");
...
navigator.mediaDevices.enumerateDevices()
    .then(devices => {
        ...
        // Camera 3
        if (cameras.length >= 3) {
            const cameraId3 = cameras[2].deviceId;
            const videoConstraints3 = {
                ...
                // width: { min: 640, ideal: 1920, max: 1920 },
                // height: { min: 400, ideal: 1080 },
                deviceId: { exact: cameraId3 },
            };

            navigator.mediaDevices.getUserMedia({ video: videoConstraints3, audio: false })
                .then(stream3 => {
                    localStream3 = stream3;
                    var mediaTracks3 = stream3.getTracks();

                    // for (i = 0; i < mediaTracks3.length; i++) {
                    //     // console.log(mediaTracks[i]);
                    // }

                    localVideo3.srcObject = stream3;
                    localVideo3.muted = true;

                    window.stream3 = stream3; // make variable available to browser console

                    const audioTracks3 = stream3.getAudioTracks();
                    const videoTracks3 = stream3.getVideoTracks();

                    btnToggleAudio3.onclick = function () {
                        audioTracks3[0].enabled = !audioTracks3[0].enabled;
                        btnToggleAudio3.innerHTML = audioTracks3[0].enabled ? '3rd Audio Mute' : '3rd Audio Unmute';
                    };

                    btnToggleVideo3.onclick = function () {
                        videoTracks3[0].enabled = !videoTracks3[0].enabled;
                        btnToggleVideo3.innerHTML = videoTracks3[0].enabled ? '3rd Video Off' : '3rd Video On';
                    };
                })
                .catch(error => {
                    console.error('Error accessing webcam 3:', error);
                });
        }
        else {
            // If there's only one camera, hide the second camera controls
            if (document.getElementById('controls2')) document.getElementById('controls2').style.display = 'none';
        }

        // Camera 4
        if (cameras.length >= 4) {
            const cameraId4 = cameras[3].deviceId;
            const videoConstraints4 = {
                ...
                // width: { min: 640, ideal: 1920, max: 1920 },
                // height: { min: 400, ideal: 1080 },
                deviceId: { exact: cameraId4 },
            };

            navigator.mediaDevices.getUserMedia({ video: videoConstraints4, audio: false })
                .then(stream4 => {
                    localStream4 = stream4;
                    var mediaTracks3 = stream4.getTracks();

                    // for (i = 0; i < mediaTracks3.length; i++) {
                    //     // console.log(mediaTracks[i]);
                    // }

                    localVideo4.srcObject = stream4;
                    localVideo4.muted = true;

                    window.stream4 = stream4; // make variable available to browser console

                    const audioTracks4 = stream4.getAudioTracks();
                    const videoTracks4 = stream4.getVideoTracks();

                    btnToggleAudio4.onclick = function () {
                        audioTracks4[0].enabled = !audioTracks4[0].enabled;
                        btnToggleAudio4.innerHTML = audioTracks4[0].enabled ? '3rd Audio Mute' : '3rd Audio Unmute';
                    };

                    btnToggleVideo4.onclick = function () {
                        videoTracks4[0].enabled = !videoTracks4[0].enabled;
                        btnToggleVideo4.innerHTML = videoTracks4[0].enabled ? '3rd Video Off' : '3rd Video On';
                    };
                })
                .catch(error => {
                    console.error('Error accessing webcam 4:', error);
                });
        }
        ...
        if (cameras.length >= 5) {
            const cameraId5 = cameras[4].deviceId;
            const videoConstraints4 = {
                frameRate: { ideal: 10, max: 12 } ,
                // width: { max: 640 },
                // height: { max: 480 },
                ...
                // width: { min: 640, ideal: 1920, max: 1920 },
                // height: { min: 400, ideal: 1080 },
                deviceId: { exact: cameraId5 },
            };

            navigator.mediaDevices.getUserMedia({ video: videoConstraints4, audio: false })
                .then(stream5 => {
                    localStream5 = stream5;
                    var mediaTracks3 = stream5.getTracks();

                    // for (i = 0; i < mediaTracks3.length; i++) {
                    //     // console.log(mediaTracks[i]);
                    // }

                    localVideo5.srcObject = stream5;
                    localVideo5.muted = true;

                    window.stream5 = stream5; // make variable available to browser console

                    const audioTracks5 = stream5.getAudioTracks();
                    const videoTracks5 = stream5.getVideoTracks();

                    btnToggleAudio5.onclick = function () {
                        audioTracks5[0].enabled = !audioTracks5[0].enabled;
                        btnToggleAudio5.innerHTML = audioTracks5[0].enabled ? '3rd Audio Mute' : '3rd Audio Unmute';
                    };

                    btnToggleVideo5.onclick = function () {
                        videoTracks5[0].enabled = !videoTracks5[0].enabled;
                        btnToggleVideo5.innerHTML = videoTracks5[0].enabled ? '3rd Video Off' : '3rd Video On';
                    };
                })
                .catch(error => {
                    console.error('Error accessing webcam 5:', error);
                });
        }
        else {
            // If there's only one camera, hide the second camera controls
...
function createOfferer(peerUsername, localScreenSharing, remoteScreenSharing, receiver_channel_name) {
    ...
    if (!localScreenSharing && !remoteScreenSharing) {
        ...
        if (peerUsername === "robot") {
            ...
            // console.log('remoteVideo3: ', remoteVideo1.video3);

            setOnTrack2(peer, remoteVideo1.video1, remoteVideo1.video2, remoteVideo1.video3,  remoteVideo1.video4, remoteVideo1.video5, remoteVideo1.video6);
        ...
        peer.oniceconnectionstatechange = () => {
            ...
            if (iceConnectionState === "failed" || iceConnectionState === "disconnected" || iceConnectionState === "closed") {
                ...
                if (peerUsername === "robot") {
                    ...
                    removeVideo(remoteVideo1.video3); 
                    removeVideo(remoteVideo1.video4);                   
                    removeVideo(remoteVideo1.video5); 
                    removeVideo(remoteVideo1.video6);                   
...
function createVideo(peerUsername) {
    ...
    var leftPortionTop = document.querySelector('#left_portion_top');
    var leftPortionMiddle = document.querySelector('#left_portion_middle');
    var leftPortionBottom = document.querySelector('#left_portion_back');
    var rightPortion = document.querySelector('#right_portion');



    // var videoContainerForOthers = document.querySelector('#remote-user_others');
    ...
    if (peerUsername === "robot") {
        ...
        var remoteVideo5 = document.createElement('video');
        remoteVideo5.id = `${peerUsername}-video5`;
        remoteVideo5.autoplay = true;
        remoteVideo5.playsinline = true;
        // remoteVideo1.ondblclick = toggleFullscreen(remoteVideo1)
        ...
        var remoteVideo3 = document.createElement('video');
        remoteVideo3.id = `${peerUsername}-video3`;
        remoteVideo3.autoplay = true;
        remoteVideo3.playsinline = true;

        var remoteVideo4 = document.createElement('video');
        remoteVideo4.id = `${peerUsername}-video4`;
        remoteVideo4.autoplay = true;
        remoteVideo4.playsinline = true;
        
        var remoteVideo6 = document.createElement('video');
        remoteVideo6.id = `${peerUsername}-video6`;
        remoteVideo6.autoplay = true;
        remoteVideo6.playsinline = true;
        ...
        pTag2.textContent = peerUsername + ' front';
        pTag3.textContent = peerUsername + ' right';
        pTag4.textContent = peerUsername + ' left';
        pTag5.textContent = peerUsername + ' rear view ';
        pTag6.textContent = peerUsername + ' back';
        ...
        videoWrapper1.classList.add("robot-front");
        ...
        videoWrapper2.classList.add("robot-left");

        videoWrapper2.appendChild(pTag2);
        videoWrapper2.appendChild(remoteVideo2);

        // // wrapper for the third video element
        var videoWrapper3 = document.createElement('div');
        videoWrapper3.classList.add("remote-user");
        videoWrapper3.classList.add("robot-right");

        videoWrapper3.appendChild(pTag3);
        videoWrapper3.appendChild(remoteVideo3);

        // // wrapper for the 4th video element
        var videoWrapper4 = document.createElement('div');
        videoWrapper4.classList.add("remote-user");
        videoWrapper4.classList.add("robot-back");

        videoWrapper4.appendChild(pTag4);
        videoWrapper4.appendChild(remoteVideo4);


        var videoWrapper5 = document.createElement('div');
        videoWrapper5.classList.add("remote-user");
        videoWrapper5.classList.add("robot-main");

        videoWrapper5.appendChild(pTag5);
        videoWrapper5.appendChild(remoteVideo5);



        var videoWrapper6 = document.createElement('div');
        videoWrapper6.classList.add("remote-user");
        videoWrapper6.classList.add("robot-front");

        videoWrapper6.appendChild(pTag6);
        videoWrapper6.appendChild(remoteVideo6);

        // add the wrappers to the video container
        // videoContainer.appendChild(videoWrapper1);
        // videoContainer.appendChild(videoWrapper2);
        // videoContainer.appendChild(videoWrapper3);

        // videoContainer.insertBefore(videoWrapper4, videoContainer.firstChild);
        // videoContainer.insertBefore(videoWrapper3, videoContainer.firstChild);
        // videoContainer.insertBefore(videoWrapper2, videoContainer.firstChild);
        // videoContainer.insertBefore(videoWrapper1, videoContainer.firstChild);

        leftPortionTop.appendChild(videoWrapper2)
        leftPortionMiddle.appendChild(videoWrapper4)
        leftPortionMiddle.appendChild(videoWrapper3)
        leftPortionBottom.appendChild(videoWrapper6)

        // var videoWrapper1Clone = videoWrapper1.cloneNode(true);
        // var videoWrapper4Clone = videoWrapper4.cloneNode(true);

        rightPortion.appendChild(videoWrapper1)
        rightPortion.appendChild(videoWrapper5)

        


        // [remoteVideo1, remoteVideo2, remoteVideo3, remoteVideo4, remoteVideo5, remoteVideo6].forEach(video => {
        //     video.addEventListener('dblclick', function() {
        //         toggleFullScreen(video);
        //     });
        // });
        return { video1: remoteVideo1, video2: remoteVideo2, video3: remoteVideo3, video4: remoteVideo4, video5: remoteVideo5, video6: remoteVideo6};
    } else {
        ...
        videoWrapper.classList.add(peerUsername);
...
function setOnTrack2(peer, remoteVideo1, remoteVideo2, remoteVideo3, remoteVideo4, remoteVideo5, remoteVideo6) {
    console.log('Setting ontrack:');
    ...
    peer.onaddstream = (event) => {
        ...
        if (event.stream) {
            ...
            } else if (remoteVideo2.srcObject === null) {
                remoteVideo2.srcObject = event.stream;
                console.log("Set on Track Remote Video 2: ", event.stream.id);
            } else if (remoteVideo3.srcObject === null) {
                remoteVideo3.srcObject = event.stream;
                console.log("Set on Track Remote Video 3: ", event.stream.id);
            } 
            else if (remoteVideo4.srcObject === null) {
                remoteVideo4.srcObject = event.stream;
                console.log("Set on Track Remote Video 4: ", event.stream.id);
            }
            else if (remoteVideo6.srcObject === null) {
                remoteVideo6.srcObject = event.stream;
                remoteVideo5.srcObject = event.stream;
                console.log("Set on Track Remote Video 4: ", event.stream.id);
            }
            else {
                console.log("All remote videos are occupied.");
            }
        }
    }
}
...
function addLocalTracks(peer, localScreenSharing) {
    if (!localScreenSharing) {
        ...
        localStream3.getTracks().forEach(track => {
            console.log('Adding 3rd localStream track: ', track);
            peer.addTrack(track, localStream3);
        });
        localStream4.getTracks().forEach(track => {
            console.log('Adding 3rd localStream track: ', track);
            peer.addTrack(track, localStream4);
        });
        localStream5.getTracks().forEach(track => {
            console.log('Adding 3rd localStream track: ', track);
            peer.addTrack(track, localStream5);
        });
